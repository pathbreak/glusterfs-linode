#!/usr/bin/python
"""
From http://people.canonical.com/~rsalveti/iozone-test-omap-mmc-improvement/postprocessing.py
...
"""
import os, sys, optparse, math, time
...
class IOzoneAnalyzer(object):
    ...
    * Summary of throughput for all record sizes
    ...
    def __init__(self, list_files, output_dir):
        ...
        print "Results will be stored in %s" % output_dir
    ...
    def report(self, overall_results, record_size_results, file_size_results):
        ...
        print ""
        print "TABLE:  SUMMARY of ALL FILE and RECORD SIZES                        Results in MB/sec"
        print ""
        print "FILE & RECORD  INIT    RE              RE    RANDOM  RANDOM  BACKWD   RECRE  STRIDE    F       FRE     F       FRE"
        print "SIZES (KB)     WRITE   WRITE   READ    READ    READ   WRITE    READ   WRITE    READ    WRITE   WRITE   READ    READ"
        print "-------------------------------------------------------------------------------------------------------------------"
        for result_line in overall_results:
            print "ALL            %-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s" % tuple(result_line)
        print ""

        print "DRILLED DATA:"

        print ""
        print "TABLE:  RECORD Size against all FILE Sizes                          Results in MB/sec"
        print ""
        print "RECORD    INIT    RE              RE    RANDOM  RANDOM  BACKWD   RECRE  STRIDE    F       FRE     F       FRE "
        print "SIZE (KB) WRITE   WRITE   READ    READ    READ   WRITE    READ   WRITE    READ    WRITE   WRITE   READ    READ"
        print "--------------------------------------------------------------------------------------------------------------"

        foutput_path = os.path.join(self.output_dir, '2d-datasource-file')
        if os.path.isfile(foutput_path):
            os.unlink(foutput_path)
        foutput = open(foutput_path, 'w')
        print "%s" % (foutput_path)
        for result_line in record_size_results:
            print "%-10s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s" % tuple(result_line)
            foutput.write("%-10s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s\n" % tuple(result_line))
        foutput.close()

        print ""

        print ""
        print "TABLE:  FILE Size against all RECORD Sizes                          Results in MB/sec"
        print ""
        print "RECORD    INIT    RE              RE    RANDOM  RANDOM  BACKWD   RECRE  STRIDE    F       FRE     F       FRE "
        print "SIZE (KB) WRITE   WRITE   READ    READ    READ   WRITE    READ   WRITE    READ    WRITE   WRITE   READ    READ"
        print "--------------------------------------------------------------------------------------------------------------"
        ...
        routput = open(routput_path, 'w')
        print "%s" % (routput_path)
        for result_line in file_size_results:
            print "%-10s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s" % tuple(result_line)
            routput.write("%-10s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s%-8s\n" % tuple(result_line))
        routput.close()
    ...
    def report_comparison(self, record, file):
        ...
        (file_size, file_improvements, file_regressions,
         file_total) = file
        print "ANALYSIS of DRILLED DATA:"

        print ""
        print "TABLE:  RECsize Difference between runs                            Results are % DIFF"
        print ""
        print "RECORD    INIT    RE              RE    RANDOM  RANDOM  BACKWD   RECRE  STRIDE    F       FRE     F       FRE "
        print "SIZE (KB) WRITE   WRITE   READ    READ    READ   WRITE    READ   WRITE    READ    WRITE   WRITE   READ    READ"
        print "--------------------------------------------------------------------------------------------------------------"
        for result_line in record_size:
            print "%-10s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s" % tuple(result_line)
        print "REGRESSIONS: %d (%.2f%%)    Improvements: %d (%.2f%%)" % (record_regressions,
        ...
        print ""
        print "TABLE:  FILEsize Difference between runs                           Results are % DIFF"
        print ""
        print "RECORD    INIT    RE              RE    RANDOM  RANDOM  BACKWD   RECRE  STRIDE    F       FRE     F       FRE "
        print "SIZE (KB) WRITE   WRITE   READ    READ    READ   WRITE    READ   WRITE    READ    WRITE   WRITE   READ    READ"
        print "--------------------------------------------------------------------------------------------------------------"
        for result_line in file_size:
            print "%-10s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s%-8.6s" % tuple(result_line)
        print "REGRESSIONS: %d (%.2f%%)    Improvements: %d (%.2f%%)" % (
                     file_regressions,
...
class IOzonePlotter(object):
    ...
    def __init__(self, results_file, output_dir):
        ...
        if not os.path.isfile(results_file):
            print "Invalid file %s provided, disabling graph generation", results_file
            self.active = False
    ...
    def plot_2d_graphs(self):
        ...
        for index, label in zip(range(2, 15), _LABELS[2:]):
            ...
            commands_file.close()
            os.system("%s %s" % (self.gnuplot, commands_path))
